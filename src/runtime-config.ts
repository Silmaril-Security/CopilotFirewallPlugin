...
export function configurationPath(env: RuntimeEnv = process.env): string {
  ...
  const copilotHome = nonEmpty(env.COPILOT_HOME)
    ?? path.join(nonEmpty(env.HOME) ?? homedir(), ".copilot");
  return path.join(copilotHome, "silmaril-firewall.json");
}

export function copilotHome(env: RuntimeEnv = process.env): string {
  return nonEmpty(env.COPILOT_HOME)
    ?? path.join(nonEmpty(env.HOME) ?? homedir(), ".copilot");
}
